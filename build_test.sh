...
# Simple build script for testing automated architecture evolution
# This compiles just the essential components for testing

echo "Building automated architecture evolution test..."

# Create include directory structure
mkdir -p includes

# Create a simplified header with just what we need
cat > includes/ggml-distributed-cognitive.h << 'EOF'
#pragma once
#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>
...
# Create a minimal ggml stub for testing
cat > ggml_stub.c << 'EOF'
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
# Create a modified version of the distributed cognitive source with the correct includes
sed 's/#include "ggml-distributed-cognitive.h"/#include "includes\/ggml-distributed-cognitive.h"/' ggml/src/ggml-distributed-cognitive.c > distributed-cognitive-modified.c

# Remove the includes that don't exist in our test
sed -i 's/#include "ggml-distributed-communication.h"/\/\/ #include "ggml-distributed-communication.h"/' distributed-cognitive-modified.c
sed -i 's/#include "ggml-cognitive-tensor.h"/\/\/ #include "ggml-cognitive-tensor.h"/' distributed-cognitive-modified.c
sed -i 's/#include "ggml-cogfluence.h"/\/\/ #include "ggml-cogfluence.h"/' distributed-cognitive-modified.c
sed -i 's/#include "ggml-opencog.h"/\/\/ #include "ggml-opencog.h"/' distributed-cognitive-modified.c

# Update test to use correct include
sed 's/#include "ggml\/include\/ggml-distributed-cognitive.h"/#include "includes\/ggml-distributed-cognitive.h"/' test_automated_architecture_evolution.c > test-modified.c

# Compile the test
gcc -I. -Iincludes -DGGML_API= \
    ggml_stub.c \
    distributed-cognitive-modified.c \
    test-modified.c \
    -o test_automated_architecture_evolution \
    -lm

if [ $? -eq 0 ]; then
    echo "Build successful! Running test..."
    ./test_automated_architecture_evolution
else
    echo "Build failed!"
    exit 1
fi